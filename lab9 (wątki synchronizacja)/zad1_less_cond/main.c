...
void* elf_thread(void* arg){
    ...
    while (true){
        lock_mutex(elves_ready_mutex[elf->name]); ///tylko blokuje, santa odblokowuje
...
void* reindeer_thread(void* arg){
    ...
    while (true){
        lock_mutex(reindeer_ready_mutex[reindeer->name]); ///tylko blokuje, santa odblokowuje
    ...
    while (true){
        ...
        if (all_reindeer_ready) {
            ...
            for___(ALL_REINDEER, reindeer) {
                unlock_mutex(reindeer_ready_mutex[reindeer]); /// odblokowanie czekajacych reniferow
            }
        }
        unlock_mutex(all_reindeer_ready_mutex);
        ...
        if(elves_at_santa){
            ...
            for___(ELVES_WAITING, elf){
                unlock_mutex(elves_ready_mutex[waiting_elves_names[elf]]); /// odblokowanie czekajacych elfow
            }
